...
function NextDividendCard() {
  return (
    <div className="rounded-2xl bg-[#14141A] border border-brand/20 p-3 sm:p-4 shadow-[0_16px_40px_rgba(0,0,0,0.6),0_0_24px_rgba(51,190,84,0.15)]">
      <div className="flex items-center gap-2 mb-2">
        ...
        <span className="text-[9px] sm:text-[10px] font-mono font-medium text-[#8A8A92] tracking-wider">
          NEXT DIVIDEND
        </span>
      </div>
      <p className="text-lg sm:text-xl font-mono font-semibold text-white">€31.23</p>
      <p className="text-[10px] sm:text-[11px] text-[#8A8A92] mt-1">JPM · highest payer</p>
    </div>
  )
}
  ...
  return (
    <div className="rounded-2xl bg-[#14141A] border border-brand/20 p-3 sm:p-4 shadow-[0_16px_40px_rgba(0,0,0,0.6),0_0_24px_rgba(51,190,84,0.15)]">
      <div className="flex items-center justify-between mb-2">
        ...
        <span className="px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-full bg-brand text-[8px] sm:text-[9px] font-mono font-bold text-[#0A0A0F] tracking-wider">
          BUY
        </span>
      </div>
      <p className="text-lg sm:text-xl font-mono font-semibold text-white">NVDA</p>
      <p className="text-[10px] sm:text-[11px] text-[#8A8A92] mt-1">100 shares · €92.90</p>
    </div>
  )
}
...
function TradingActivityCard() {
  return (
    <div className="rounded-2xl bg-[#14141A] border border-brand/20 p-3 sm:p-4 shadow-[0_16px_40px_rgba(0,0,0,0.6),0_0_24px_rgba(51,190,84,0.15)]">
      <p className="text-[9px] sm:text-[10px] font-mono font-medium text-[#8A8A92] tracking-wider mb-1">
        TRADING ACTIVITY
      </p>
      <div className="flex items-end gap-1.5 mb-2">
        <span className="text-xl sm:text-2xl font-mono font-semibold text-white">73%</span>
        <span className="text-[10px] sm:text-[11px] text-[#8A8A92] pb-0.5">win rate</span>
      </div>
      <div className="flex gap-[3px]" aria-hidden="true">
        {HEATMAP_CELLS.map((opacity, i) => (
          <div
            key={i}
            className="h-3 sm:h-4 flex-1 rounded-[3px]"
  ...
  return (
    <div className="rounded-2xl bg-[#14141A] border border-brand/20 p-3 sm:p-4 shadow-[0_16px_40px_rgba(0,0,0,0.6),0_0_24px_rgba(51,190,84,0.15)]">
      ...
      <div className="flex items-end gap-2">
        <span className="text-xl sm:text-2xl font-mono font-semibold text-brand">+€422.53</span>
        <TrendingUp className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-brand mb-1" />
      </div>
      <p className="text-[10px] sm:text-[11px] text-[#8A8A92] mt-1">19 winning positions</p>
    </div>
  )
}
...
export function ProductDemo() {
  ...
  const tlX = useTransform(scrollYProgress, [0.25, 0.48], [80, 0])
  const tlY = useTransform(scrollYProgress, [0.25, 0.48], [40, 0])
  const tlRotate = useTransform(scrollYProgress, [0.25, 0.48], [0, -4])

  // Top-right: NVDA
  const trOpacity = useTransform(scrollYProgress, [0.28, 0.45], [0, 1])
  const trX = useTransform(scrollYProgress, [0.28, 0.51], [-80, 0])
  const trY = useTransform(scrollYProgress, [0.28, 0.51], [40, 0])
  ...
  const blX = useTransform(scrollYProgress, [0.31, 0.54], [80, 0])
  const blY = useTransform(scrollYProgress, [0.31, 0.54], [-40, 0])
  ...
  const brX = useTransform(scrollYProgress, [0.34, 0.57], [-80, 0])
  const brY = useTransform(scrollYProgress, [0.34, 0.57], [-40, 0])
  ...
  return (
    ...
    >
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl relative">
        ...
        {/* Demo layout — same choreography on mobile and desktop, sizes adjusted for viewport.
            Mobile/sm: pad top+bottom so satellites sit outside the dashboard instead of covering it. */}
        <div className="relative pt-24 pb-24 lg:pt-0 lg:pb-0">
          {/* Top-left: Next Dividend */}
          <motion.div
            style={{ x: tlX, y: tlY, rotate: tlRotate, opacity: tlOpacity }}
            className="absolute -left-1 top-0 sm:left-0 sm:top-2 lg:-left-2 lg:top-8 xl:left-4 z-20 w-[44vw] sm:w-52 lg:w-56"
          >
            <NextDividendCard />
          </motion.div>
          ...
          <motion.div
            ...
            className="absolute -right-1 top-0 sm:right-0 sm:top-2 lg:-right-2 lg:top-8 xl:right-4 z-20 w-[44vw] sm:w-52 lg:w-56"
          >
            <NvdaCard />
          </motion.div>
          ...
          <motion.div
            ...
            className="absolute -left-1 bottom-0 sm:left-0 sm:bottom-2 lg:-left-2 lg:bottom-2 xl:left-4 z-20 w-[44vw] sm:w-52 lg:w-56"
          >
            <TradingActivityCard />
          </motion.div>
          ...
          <motion.div
            ...
            className="absolute -right-1 bottom-0 sm:right-0 sm:bottom-2 lg:-right-2 lg:bottom-2 xl:right-4 z-20 w-[44vw] sm:w-52 lg:w-56"
          >
            <RealizedPnlCard />
          </motion.div>
